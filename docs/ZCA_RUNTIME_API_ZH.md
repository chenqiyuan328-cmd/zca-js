...
本文档对应浏览器注入版 `zca-runtime.js`，适用于 Android WebView、Flutter WebView 或浏览器控制台。当前 Runtime 实现个人账号的同设备登录、手机号查找、文字/图片任务发送、ACK 和单向删除，不包含二维码自动化或验证码绕过。
...
| `sendTaskImage(uid, caption, imageURL)` | 从公共 HTTPS 地址读取图片并发送带说明的图片。          |
...
| `link`         | 否   | 单独配置的 HTTPS 链接，发送时追加到文字或图片说明。    |
| `image`        | 否   | 公共 HTTPS 图片地址；支持 JPEG、PNG、WebP，最大 10MB。 |
...
| `IMAGE_URL_NOT_ALLOWED`     | 图片不是公共 HTTPS 地址，或地址指向本机/内网。 |
| `IMAGE_DOWNLOAD_FAILED`     | 图片下载或安全重定向校验失败。                 |
| `INVALID_IMAGE_TYPE`        | 图片格式或尺寸无效。                           |
| `INVALID_IMAGE_SIZE`        | 图片为空或超过 10MB。                          |
| `IMAGE_SERVICE_UNAVAILABLE` | 当前 Zalo 会话没有提供图片服务。               |
| `INVALID_UPLOAD_RESPONSE`   | Zalo 图片上传没有返回有效结果。                |
...
10. 当前支持一对一文字和单图消息，不支持普通文件、多图和群消息。
